...
// BestEffortFallbackFunc is called when a wrapped action returns an error.
...
//
// The wrapped action keeps its original Name behavior. When the wrapped action
// succeeds, its output is returned normally. When it returns an error, fallback
// is called with the original input and error, then the wrapped action's output
// is returned with a nil error.
// This allows a Workflow to continue through the Success direction even when
// the wrapped action failed in a non-critical way.
